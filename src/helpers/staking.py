...
class Staking:
    ...
    def get_staked_token_price(self):
        if self.is_LP:
            ...
            _token0_decimals = _token0.get_decimals()
            _token1_decimals = _token1.get_decimals()
            ...
            _lp_decimals = _pair.get_decimals()
    ...
    def get_current_apr(self):
        ...
        _rewards_token_decimal = Token(self.rewards_token).get_decimals()
        _rewards_token_price = self.get_rewards_token_price()

        _total_staked_tokens = self.get_total_staked()
        _staked_contract_decimals = self.get_staked_contract_decimals()
